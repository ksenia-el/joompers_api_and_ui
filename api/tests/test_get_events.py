...
class TestEvents:
    ...
    def test_get_events(self, user_logged_in_session_fixture, data_test):
    ...
    @pytest.mark.parametrize("data_test", TestData.valid_user_credentials)
    def test_post_events(self, user_logged_in_session_fixture, data_test):
        user_auth_data = UserAccount(user_logged_in_session_fixture)
        api = Events(user_auth_data.session)
        response_body, status = api.get_events()
    ...
    @pytest.mark.parametrize("data_test", TestData.valid_user_credentials)
    def test_post_events_wrong_id(self, user_logged_in_session_fixture, data_test):
        user_auth_data = UserAccount(user_logged_in_session_fixture)
        api = Events(user_auth_data.session)
        response_body, status = api.get_events()
