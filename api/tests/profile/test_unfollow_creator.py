...
class PreTestSetUpUnfollow:
    """
       ...
       """
    def test_start_follow_creator(self, user_logged_in_session_fixture):
        ...
        profile_api.follow_user(valid_data)
...
class TestUnfollowCreator:
    ...
    def test_unfollow_creator_profile_positive(self, user_logged_in_session_fixture):
        ...
        profile_api = Profile(authenticated_session)
        response_body, status_code = profile_api.unfollow_user(valid_data)
    ...
    def test_unfollow_yourself_negative(self, user_logged_in_session_fixture):
        ...
        profile_api = Profile(authenticated_session)
        response_body, status_code = profile_api.unfollow_user(data_with_current_user_profile_id)
        error_message = response_body.get("message")

        assert status_code == 400, f"Expected 400 for unfollowing yourself, got {status_code} instead"
        assert error_message == "You don't follow this user"
    ...
    def test_unfollow_creator_with_non_existing_user_profile_id_negative(self, user_logged_in_session_fixture):
        ...
        profile_api = Profile(authenticated_session)
        response_body, status_code = profile_api.unfollow_user(data_non_existing_user_profile_id)
